...
- `GROQ_API_KEY` set; one live STAR evaluation already verified in preflight
...
## 6. Intelligent feedback (~25s)

**Say:**  
“After the attempt is saved, Ameego evaluates only the confirmed transcript against STAR. Every score comes with explanation and evidence that must appear in the transcript. It does not judge emotion, honesty, or hireability—and it is not an official grade.”
...
“We used Codex to ship the empty repository into a Next.js app, wire the interview simulator and evidence-grounded STAR evaluation, build attempt comparison, and run a full release audit with storage and accessibility fixes—recorded in our Codex workflow log.”
...
**Say:**  
“Ameego: learn STAR, practice, get grounded AI feedback, retry, and compare—on this device.”
...
If over time: skip second full interview on camera; show Progress comparison from a rehearsal, and say both attempts were evaluated with the same STAR rubric in the same scenario.
