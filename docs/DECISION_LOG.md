...
## Decision 039 - Center Visible Campus Artwork Within Each Plot

Date: 2026-07-18

Status: Accepted

Context: The five campus overlays used a regular row-based coordinate grid, but
the background's open plots and the transparent margins inside each building
PNG are not identical. Equal wrapper coordinates therefore did not place the
visible buildings at the visual centers of their designated plots.

Decision: Keep the existing proportional map coordinate system and original
building widths, but assign each overlay a plot-specific percentage anchor.
Use small horizontal and vertical offsets to compensate for the supplied
assets' different transparent bounds and the illustrated plot edges.

Alternatives considered: Resizing or cropping the assets, applying per-image
transforms, changing the background artwork, or forcing both rows onto identical
top coordinates.

Reason: Position-only anchors improve balance without changing artwork scale,
perspective, interaction geometry, shadows, animation, navigation, or backend
behavior. Accounting for visible pixels rather than canvas centers produces the
cleanest alignment within the irregular illustrated plots.

Consequences: The campus reads as a more symmetrical composition around the
plaza. Labels remain attached and horizontally centered beneath their buildings,
and responsive scaling continues to use the same single coordinate system.

---

## Decision 040 - Derive Player XP from Existing Progress Records

Date: 2026-07-18

Status: Accepted

Context: The headers displayed fixed `XP 0000` and `LV 01`, while the Progress
Library and Academy dashboard independently calculated progress from the same
local lesson, exercise, and interview stores. Their level definitions also
differed because Academy rank position was being exposed as level.

Decision: Use one pure player-progress calculator as the source of truth. Award
the configured XP for known completed Academy lessons, the existing full-course
bonus, the completed STAR exercise, completed mock interviews, and validated
feedback. Calculate Level as `floor(XP / 500) + 1`, keep rank position separate,
and derive values on demand from persisted records instead of writing a second
XP balance. Notify same-tab subscribers after existing storage mutations and
listen to browser storage events for cross-tab changes.

Alternatives considered: Persisting a second mutable XP total, calculating XP
independently in each header, polling local storage, treating Academy rank index
as Level, or adding bonus XP for derived badges and missions without a durable
claim record.

Reason: Deterministic derivation prevents double awards and drift, honors the
existing persistence boundary, and makes historical records immediately
compatible. Activity that unlocks badges or fulfills missions already earns its
configured lesson, exercise, or interview XP; a separate reward ledger can be
added later if missions gain an explicit claim workflow.

Consequences: XP and Level update immediately in both headers, persist between
sessions, and agree with the Progress Library and Academy dashboard. Corrupt
local records safely fall back to a new-learner header state, while the detailed
screens retain their existing error handling.

---

## Decision 041 - Use Peer Sections and Sticky Navigation for Settings

Date: 2026-07-18

Status: Accepted

Context: Settings nested Audio, Privacy, Data, Permissions, and About inside one
Profile panel with a narrow fixed-width frame. This visually implied that every
setting belonged to Profile, constrained usable desktop width, and produced an
uneven reading hierarchy.

Decision: Make the eight requested categories peer sections in one natural page
flow. Use a sticky left sidebar on desktop and laptop widths, switch it to a
normal responsive grid on narrower screens, and keep every content section in
the document flow without an internal vertical scroll container. Track the
active anchor visually while preserving native links and focus behavior.

Alternatives considered: Tab panels that hide inactive categories, a fixed
viewport dashboard with an internally scrolling content pane, one continuous
undifferentiated form, or separate routes for each category.

Reason: Peer sections improve discoverability and allow direct anchor links,
while normal browser scrolling preserves expected keyboard, touch, history, and
accessibility behavior. Sticky navigation keeps orientation without creating a
second scrolling surface.

Consequences: Settings occupies the available browser width, categories are
visually equal, and related controls use responsive grids. The sidebar becomes
part of normal flow below 820px so it cannot squeeze the content column. No
data, permission, API, or persistence contract changes.

---

## Decision 042 - Follow the Reference Hierarchy Over the Night Hero

Date: 2026-07-18

Status: Accepted

Context: The updated night scene and interface reference show the welcome copy
on the left, goal card on the right, and full-width XP and quick actions below.
The first integration reserved the right side completely, which no longer
matched the updated reference hierarchy.

Decision: Render the replacement image as the hero's centered, cover-sized
background. Restore a two-column greeting-and-goal row, followed by full-width
XP and four-column quick actions. Use a strong-to-light horizontal overlay plus
a lower overlay for the controls. Reduce actions to two columns at 1100px and
stack the main row below 820px.

Alternatives considered: Applying the image to the whole dashboard, placing it
in a separate decorative panel, stretching it to match the hero dimensions,
reserving the entire right side, placing the image in a separate panel, or
darkening the entire artwork equally at every viewport.

Reason: This placement follows the supplied UI reference, keeps the information
hierarchy immediately scannable, preserves the academy and fountain as visible
landmarks between translucent and spaced controls, and avoids distortion.

Consequences: The Main Building hero is more compact and uses four equal desktop
quick actions. The goal returns to the right column, controls stack responsively,
and the rest of the dashboard retains its existing background.

---

## Decision 043 - Use One Fixed Cover Scene for the Long Dashboard

Date: 2026-07-18

Status: Accepted

Context: The Main Building dashboard is substantially taller than the uploaded
star-field image. Scaling the image to the full document height would distort
the pixel artwork, while vertical repetition could expose an obvious seam
between copies.

Decision: Apply the star field to the Main Building page container with centered
cover sizing, no repetition, and fixed attachment. Place a subtle dark gradient
in the same background stack, make the hero transparent, and retain only a
soft radial overlay behind its welcome copy.

Alternatives considered: Stretching one image to the document height, repeating
the image vertically, retaining the separate academy hero image, applying the
star field globally to every route, or adding separate backgrounds to each
dashboard section.

Reason: A fixed cover layer remains continuous for the entire browser-owned
scroll without seams or distortion. Scoping it to the dashboard container keeps
the shared header and every other route unchanged.

Consequences: The same star scene remains behind all Main Building content as
the learner scrolls. Panels provide primary contrast, the hero overlay fades
naturally, and there is no visible boundary between hero and dashboard body.

---

## Decision 044 - Separate Hero Scenery from the Scrolling Dashboard Scene

Date: 2026-07-18

Status: Accepted

Context: Applying the star field behind the Welcome Back content removed the
updated academy composition that defines the Main Building entrance. The desired
result uses both supplied artworks at different levels of the page.

Decision: Keep the academy night scene as the hero’s own centered cover
background and keep the fixed star field on the parent dashboard page. Because
the hero paints an opaque background, the star field appears only after the hero
ends and continues behind the remaining page content.

Reason: Section ownership preserves the Main Building entrance design while
retaining a continuous scrolling backdrop for quests, achievements, and other
dashboard panels. It requires no markup or behavior changes.

Consequences: The hero and dashboard body have distinct but coordinated night
scenes. The hero retains its reference composition and overlays; the star field
continues from directly below its lower border to the bottom of the page.

---

## Decision 045 - Build the Progress Hero from the Existing Snapshot

Date: 2026-07-18

Status: Accepted

Context: The Progress route’s title was server-rendered separately from the
client dashboard, so it could not show the requested real statistics, current
goal, recommendation, or streak. Keeping the existing Statistics section after
adding the same values to the hero would also duplicate information.

Decision: Move the title composition into a reusable client-side ProgressHero
rendered after the existing progress snapshot loads. Use completed lesson count,
simulation count, evaluated simulation count, skill-progress count,
recommendedNext, and currentStreak directly. Render the same hero with zero
values above the existing new-learner empty state, and remove the duplicated
Statistics section.

Alternatives considered: Adding a second progress reader to the route, showing
fixed example counts, retaining duplicated statistics, adding new persisted
goal state, or rendering decorative cards with no relationship to learner data.

Reason: One snapshot keeps the hero honest and synchronized with the rest of the
page. Reusing current recommendation and streak data avoids new state or backend
work, while removing duplication improves hierarchy.

Consequences: The hero appears once data is ready and accurately represents
both new and active learners. Detailed XP, skill, activity, completion, attempt,
and comparison views remain below unchanged.

Alignment amendment: Center the Progress Library label, title, description, and
statistics row as one hero group. Preserve the hero background, navigation,
card styling, values, and dashboard body.

Amendment: The reference-alignment refinement keeps only the four compact
statistics inside the approximately 450px desktop hero. Current Goal, Next
Lesson, and Current Streak remain snapshot-driven but render as a separate
responsive row in the main content, leaving the castle side unobstructed.
The row uses purpose-specific premium widgets with verified snapshot metadata
and existing destinations; it does not invent estimated time, difficulty, XP
rewards, or new persisted progress.

Hero-stat refinement: Keep the four header values unchanged, but derive compact
supporting metadata from existing course and snapshot data. Match the supplied
reference with 250×100px horizontal cards, 44px left-aligned pixel icons, one
status line, a small contextual footer, and distinct top accents without adding
new product state.

---

## Decision 046 - Compact the Progress Body Without Touching the Header

Date: 2026-07-18

Status: Accepted

Context: The body panels used generous landing-page spacing and oversized empty
areas, while the requested experience is a dense RPG records dashboard. The
navigation and hero were explicitly out of scope.

Decision: Limit changes to the dashboard beneath ProgressHero. Reuse every
existing value and control while reducing body spacing, introducing consistent
icon-led accents, compressing record cards and timeline rows, and grouping the
two existing comparison selectors around a decorative center icon.

Reason: CSS and presentation-only markup can improve scanability without
changing progress calculations, persistence, routes, state, or backend behavior.

Consequences: The body is denser and more visually consistent; all previous
records, controls, empty states, attempt opening, and comparison behavior remain
available. Navigation and hero rendering are unchanged.

---

## Decision 047 - Require Confirmation for Individual Attempt Deletion

Date: 2026-07-18

Status: Accepted

Context: Saved interview records contain transcripts and may contain validated
feedback. Removing one is irreversible in browser-local storage and therefore
must not happen from a single card click.

Decision: Place a compact Delete action beside Open Attempt, then require an
accessible alert dialog that identifies the record and explains what is removed.
Perform deletion through the interview storage module, notify progress listeners,
clear UI selections referencing the record, and reload the snapshot.

Reason: Centralized deletion preserves storage invariants and cross-page XP
synchronization, while explicit confirmation prevents accidental data loss.

Consequences: Learners can remove one saved interview safely. Cancel and Escape
are non-destructive; confirmed deletion removes only the selected record and
updates the dashboard immediately.

---

## Decision 048 - Restore the Established Progress Hero Proportions

Date: 2026-07-18

Status: Accepted

Decision: Reverse only the immediately preceding hero-size reduction and return
to the established 440–500px hero, original typography and spacing, and four
250×100px statistic cards.

Reason: The project owner preferred the previous reference-matched proportions.

Consequences: The earlier Progress Library design and functionality remain
intact; only the latest proportional compaction is removed.

---

## Decision 049 - Layer the Star Field Beneath the Progress Hero

Date: 2026-07-18

Status: Accepted

Decision: Apply the supplied starry-night image to the full Progress Library
page container using fixed centered cover sizing and a dark gradient. Keep the
existing hero's opaque moonlit landscape background declaration unchanged.

Reason: A page-level fixed layer fills every body gap throughout scrolling
without stretching a single image across the document's full height or adding
visible vertical seams.

Consequences: The body becomes one continuous night scene while the navigation,
hero artwork, content layout, interactions, data, and backend remain unchanged.

---

## Decision 050 - Densify the Main Building Body Through Scoped CSS

Date: 2026-07-18

Status: Accepted

Context: The Main Building body already contains every requested feature and
truthful data point, but its generous card sizing and uniform panels weaken
hierarchy and leave excessive empty space.

Decision: Preserve the existing component markup and data flow. Apply the new
visual hierarchy only from `dashboardBody` downward using section accents,
low-opacity pixel ornaments, compact grouped cards, stronger values, timeline
connectors, and reduced-motion-safe hover feedback.

Reason: The existing semantic structure supports the full redesign without
risking behavior, data binding, routing, or the carefully composed hero.

Consequences: The body is denser and more game-like while all content, controls,
responsive breakpoints, persistence, and backend behavior remain intact.

---

## Decision 051 - Derive Courses Visual States From Existing Progress

Date: 2026-07-18

Status: Accepted

Context: The Courses page already computes the current lesson, completed lesson
IDs, unlocked order, earned phase badges, progress, XP, and time. The requested
redesign needs richer current/completed/locked presentation but no new product
state.

Decision: Reuse those existing values to apply presentation-only current badge,
current lesson, phase percentage, and quest metadata treatments. Keep all
navigation and progression decisions in their existing functions.

Reason: Deriving visual state from the existing progress snapshot prevents UI
labels from diverging from lesson access or persisted completion.

Consequences: The Courses body gains clearer hierarchy and state distinction
without changing routes, unlocking, XP, persistence, or backend behavior.

Background amendment: Reuse the existing byte-identical starry-night asset on
the global route wrapper classes for the course overview, STAR lesson, academy
lessons, and STAR exercise. Fixed centered cover sizing avoids seams or vertical
stretching on long pages, and the scoped selectors do not affect other routes.
Apply the image to the shared site shell rather than the document body because
the shell owns the opaque canvas directly behind course content.

---

## Decision 052 - Keep Avatar Persistence Inside the Existing Local Profile

Date: 2026-07-18

Status: Accepted

Context: The Settings redesign explicitly requires uploaded and preset profile
pictures, while existing learner profiles contain only name and learning focus
in the version-one local record.

Decision: Add a validated avatar value to the same learner profile record. Read
older records without an avatar as the default Scholar preset, keep the storage
key and schema version unchanged, constrain uploads to PNG, JPEG, or WebP under
1 MB, and require the existing Save Profile action before persistence.

Reason: This delivers the requested profile-picture feature without a new data
store, migration step, route, API, or backend dependency, and preserves the
existing profile save/reset/export lifecycle.

Consequences: Existing saved profiles remain valid. New saves include either a
small preset identifier or a validated image data URL; Reset All Data continues
to remove the complete local profile while Clear Progress preserves it.

---

## Decision 053 - Keep Pre-interview Refinement Inside the Preparation Stylesheet

Date: 2026-07-18

Status: Accepted

Context: Scenario, Resume, Review, and Mode Selection already have semantic
markup, working state transitions, validation, resume actions, and responsive
layout. Their visual density can be improved without changing behavior.

Decision: Implement the refinement in the scoped preparation CSS and its style
contract test. Remove only the preparation-specific rules that reconfigured and
hid the shared header navigation; do not edit MainNav, InterviewSimulator logic
or markup, active-session styles, or result styles.

Reason: The existing component structure contains every requested field,
summary value, status, and action. A scoped style-only change has the smallest
risk of affecting interview generation, permissions, progress, or saved data.

Consequences: The complete pre-interview journey is denser and easier to scan,
the global header remains consistent with other academy pages, browser scrolling
remains the only page scroll surface, and simulator/post-interview presentation
is unchanged.

---

## Decision 054 - Derive Reward-screen Status From Existing Evaluation State

Date: 2026-07-18

Status: Accepted

Context: The completed Interview Center already distinguishes idle, loading,
error, and validated evaluation states and owns the generate, retry, same-scenario
retry, and restart handlers.

Decision: Recompose only the complete-state markup and result-scoped CSS. Map
the existing states to Ready to Generate, Processing, Service Offline, and
Feedback Ready badges; reuse the saved questions and confirmed responses for the
transcript panel; wire quick actions to the existing restart handler and current
academy routes.

Reason: Presentation derived from existing state cannot drift from the API or
saved-attempt lifecycle and requires no new backend or product state.

Consequences: The completed page feels like a reward screen and exposes clearer
next actions while evaluation, validation, storage, retry, report rendering, and
navigation contracts remain intact. The global header is no longer hidden by a
results-only CSS override.

---

## Decision 055 - Reserve a Dedicated Hero Zone for Decorative Artwork

Date: 2026-07-18

Status: Accepted

Context: The short result hero placed a large absolute interviewer image inside
the same horizontal space as two minimum-width functional cards. Laptop widths
could not satisfy the cards, artwork, and oversized gap simultaneously, causing
overlap and text clipping.

Decision: Use three explicit desktop columns for completion content,
interviewer artwork, and intelligent-feedback action. At 1100px, switch to two
functional columns and hide the decorative artwork; at 820px, stack both cards.
Give every nested grid child a shrinkable width and allow long user- or
AI-generated text to wrap within its owning card.

Reason: Reserving layout space for decoration is more robust than balancing
absolute offsets against unknown viewport and content sizes. Removing the
decoration before space becomes constrained preserves the important controls
and content.

Consequences: The background remains the hero's visual anchor on wide screens,
functional cards never depend on image overlap, transcript and feedback content
remain contained, and the page retains browser-owned scrolling without changes
to data or behavior.

---

## Decision 056 - Derive Transcript Metadata From the Saved Attempt

Date: 2026-07-18

Status: Accepted

Context: The premium transcript design needs consistent confirmation, input,
date, and time metadata. Existing confirmed responses store their input mode,
while the completed attempt stores the authoritative completion timestamp; the
storage contract does not contain a separate timestamp for each response.

Decision: Display each response as Confirmed, use its existing input mode, and
format the saved attempt's completion timestamp as the card's Date and Time. Do
not invent per-response timestamps or extend the persistence schema for a
presentation-only request.

Reason: Reusing the authoritative saved values keeps the interface truthful and
avoids a storage migration or state change that would exceed the frontend-only
scope.

Consequences: Transcript cards gain aligned metadata and equal visual rhythm.
All responses from one attempt correctly share the attempt completion date and
time, while transcript persistence and parsing remain unchanged.

---

## Decision 057 - Present Saved Evidence as Full-width Horizontal Records

Date: 2026-07-18

Status: Accepted

Context: The two-column transcript cards made comparison slower and repeated a
response-count badge already communicated by the completion summary. The user
requested one clean record per confirmed answer with predictable horizontal
alignment.

Decision: Remove the count badge and use a single-column list whose desktop
rows reserve explicit columns for question number, question, answer,
confirmation, and saved date/time. Keep equal row sizing with grid auto rows;
collapse metadata to a second line on laptops and stack fields on tablets and
phones.

Reason: A record-style layout creates stable scan lines for long interview text
and lets every response use the same content grid without adding or changing
stored data.

Consequences: Saved Evidence is denser, easier to compare, and resilient to long
copy. The count remains available in the hero summary, and every response still
uses the attempt's authoritative completion timestamp.

---

## Decision 058 - Center the Guide With a Fixed Wrapper and Animate Its Image

Date: 2026-07-18

Status: Accepted

Context: The campus map scales from a fixed 1672×941 source, and the mascot must
remain exactly centered while floating. Applying both centering and animation to
one transform would let the animation overwrite the positional transform.

Decision: Anchor a non-interactive wrapper near the plaza center using
`translate(-50%, -50%)`, then animate only the child image's vertical offset.
Following visual review, use `left: 52%`, `top: 43.1%`, and a 16.4% proportional
width while keeping its z-index below all building controls.

Reason: Separating positioning from motion guarantees stable centering at every
viewport crop, preserves the original transparent artwork, and prevents the
mascot from intercepting building interactions.
