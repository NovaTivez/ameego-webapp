...
**One-sentence pitch:** Ameego is a pixel-art Pixel Communication Academy where learners study STAR storytelling, practice in an interview simulator, and get GPT-5.6 feedback grounded only in their confirmed transcript.
...
**Academy → STAR lesson → interactive STAR exercise → interview simulation → GPT-5.6 STAR feedback → recommended lesson → retry → Progress Library comparison**
...
- GPT-5.6 STAR evaluation with per-criterion scores, explanations, transcript evidence, and a recommended lesson link
...
5. Request GPT-5.6 STAR feedback on the confirmed transcript only.
...
All GPT-5.6 calls use the OpenAI Responses API from **server-only** routes (`store: false`, strict JSON Schema, timeouts, second-pass runtime validation):

| Capability                       | Route                           | Notes                                                                                              |
| -------------------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------- |
| Optional resume fact extraction  | `POST /api/interview/resume`    | Raw file used for extraction only; not saved in browser progress                                   |
...
| STAR communication evaluation    | `POST /api/interview/evaluate`  | **No offline fallback.** Requires `OPENAI_API_KEY`. Evidence must be an exact transcript substring |

Default model: `gpt-5.6` via `OPENAI_MODEL` (optional override).
...
- **AI:** OpenAI Responses API via platform `fetch` (no OpenAI SDK dependency)
...
Browser (React client)
  ...
  └── fetch → Next.js Route Handlers
        ...
        └── /api/interview/evaluate
              └── OpenAI Responses API (server-only OPENAI_API_KEY)
                    → JSON Schema output
...
| ---------------- | ---------------------------------------------------------------------- | ----------------------------------------------- |
| `OPENAI_API_KEY` | Yes for resume extraction, personalized questions, and STAR evaluation | Server-only. Never use a `NEXT_PUBLIC_` prefix. |
| `OPENAI_MODEL`   | No (defaults to `gpt-5.6`)                                             | Optional model override                         |

See [`.env.example`](.env.example). Without a key, interviews can still complete via labeled general questions, but **STAR feedback and attempt comparison cannot be demonstrated**.
...
- `OPENAI_API_KEY` stays on the server; OpenAI calls use `store: false`.
