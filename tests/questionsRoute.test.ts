import { afterEach, describe, expect, it, vi } from "vitest";

import { POST } from "@/app/api/interview/questions/route";
import { DEFAULT_INTERVIEW_SETUP } from "@/lib/interview/contracts";

const validContext = {
  setup: {
    ...DEFAULT_INTERVIEW_SETUP,
    role: "Frontend intern",
    organization: "Northstar Labs",
  },
  resumeProfile: null,
};

function makeRequest(body: string, headers: Record<string, string> = {}) {
  return new Request("http://localhost/api/interview/questions", {
    method: "POST",
    headers: { "Content-Type": "application/json", ...headers },
    body,
  });
}

describe("interview questions route", () => {
  afterEach(() => vi.unstubAllEnvs());

  it("returns 400 for malformed JSON", async () => {
    const response = await POST(makeRequest("{not json"));
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toMatchObject({
      error: expect.stringMatching(/valid JSON/i),
    });
  });

  it("returns 400 for an invalid confirmed context", async () => {
    const response = await POST(makeRequest(JSON.stringify({})));
    expect(response.status).toBe(400);
  });

  it("returns 413 for oversized request bodies without parsing them", async () => {
    const response = await POST(
      makeRequest(JSON.stringify(validContext), {
        "Content-Length": String(10 * 1024 * 1024),
      }),
    );
    expect(response.status).toBe(413);
  });

  it("returns a safe 503 when the AI provider is not configured", async () => {
    vi.stubEnv("OPENAI_API_KEY", "");
    const response = await POST(makeRequest(JSON.stringify(validContext)));
    expect(response.status).toBe(503);
    const body = (await response.json()) as { error: string };
    expect(body.error).not.toMatch(/OPENAI_API_KEY/);
  });
});
